...
func drawRowCellsLTR(ctx *DrawContext, tableX float64, colWidths []float64, textY, lineHeight float64, product models.Product, isB2B bool) {
	...
	// Quantity
	drawCellNumberSized(pdf, fmt.Sprintf("%.0f", product.Quantity), xPos, colWidths[col], textY, true, fs)
	xPos += colWidths[col]
	col++

	// Unit Price
	drawCellNumberSized(pdf, fmt.Sprintf("%.2f", product.UnitPrice), xPos, colWidths[col], textY, false, fs)
	xPos += colWidths[col]
	col++
	...
	if isB2B {
		drawCellNumberSized(pdf, fmt.Sprintf("%.2f", product.SubtotalExclVAT), xPos, colWidths[col], textY, false, fs)
		xPos += colWidths[col]
		col++
	}

	// VAT Amount
	drawCellNumberSized(pdf, fmt.Sprintf("%.2f", product.VATAmount), xPos, colWidths[col], textY, false, fs)
	xPos += colWidths[col]
	col++

	// Total
	drawCellNumberSized(pdf, fmt.Sprintf("%.2f", product.Total), xPos, colWidths[col], textY, false, fs)
}
...
func drawRowCellsRTL(ctx *DrawContext, tableX float64, colWidths []float64, textY, lineHeight float64, product models.Product, isB2B bool) {
	...
	// Quantity
	drawCellNumberSized(pdf, fmt.Sprintf("%.0f", product.Quantity), xPos, colWidths[col], textY, false, fs)
	xPos += colWidths[col]
	col++
